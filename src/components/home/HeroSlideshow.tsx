...
// titleImage: 유저가 나중에 제공할 캠페인 타이틀 PNG
// startAt: 영상 재생 시작 지점(초)
type Slide = {
  client: string;
  campaign: string;
  slug: string;
...
export default function HeroSlideshow() {
  ...
  /* ── Intro timing ──────────────────────────────────────────── */
  useEffect(() => {
    const t1 = setTimeout(() => setPhase('transition'), 1000);
    const t2 = setTimeout(() => setPhase('reel'),       1900);
  ...
  return (
    <section className="relative w-full h-[100dvh] bg-black overflow-hidden">
      ...
      {/* opacity 낮춤: 영상 위에 너무 무거운 grain이 화질을 저하시키는 효과 방지 */}
      <div
      ...
      >
        <button
          onClick={prev}
          className="absolute left-6 md:left-10 top-1/2 -translate-y-1/2 z-30 w-10 h-10 flex items-center justify-center text-white/30 hover:text-white transition-colors"
          aria-label="이전"
        >
        ...
        <button
          onClick={next}
          className="absolute right-6 md:right-10 top-1/2 -translate-y-1/2 z-30 w-10 h-10 flex items-center justify-center text-white/30 hover:text-white transition-colors"
          aria-label="다음"
        >
          <svg width="26" height="26" viewBox="0 0 24 24" fill="none">
